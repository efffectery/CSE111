...
def print_death_age(people_dict):
    ...
    for key in people_dict:
        print(people_dict[key][0], people_dict[key][-1])
...
def count_genders(people_dict):
    ...
    total_M = 0
    for key in people_dict:
        if people_dict[key][1] == "F":
            total_F += 1
        else:
            total_M += 1
    print(f"total females {total_F}, total males {total_M}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for key in marriages_dict:
        hubby = marriages_dict[key][HUSBAND_KEY_INDEX]
        wify = marriages_dict[key][WIFE_KEY_INDEX]
        year = marriages_dict[key][WEDDING_YEAR_INDEX]
        print(f"{people_dict[hubby][0]}, age {year - people_dict[hubby][2]}, >>{year}<< {people_dict[wify][0]}, age {year - people_dict[wify][2]}")
